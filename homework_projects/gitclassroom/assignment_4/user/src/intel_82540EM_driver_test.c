...
int read_test(int fd)
{
	...
	ret = read(fd, &inbuff, INT_SIZE);
	if (_usrunlikely(errno)) {
		err_msg("read_test: read failed");
		return ret;
	}

	if (_usrunlikely(ret < (ssize_t)INT_SIZE)) {
		noerr_msg("read_test: read did not read full bytes");
		return FAILURE;
	}

	printf("blink_rate value from read: %d\n", inbuff);
	return SUCCESS;
}
...
int wr_test(int fd)
{
	...
	/* read value back and print it to check if write is successful */
	ret = read(fd, &inbuff, INT_SIZE);
	if (_usrunlikely(errno)) {
		err_msg("wr_test: read failed, ret = %zu", ret);
		return FAILURE;
	}

	printf("new blink_rate from read after write: %d\n", inbuff);
