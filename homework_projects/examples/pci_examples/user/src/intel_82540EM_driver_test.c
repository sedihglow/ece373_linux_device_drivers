...
void welcome_msg()
{
	...
	printf("This program is to run tests on the intel_82540EM kernel module.\n"
	       "Read will read the LEDCTRL register and return its value.\n"
	       "Write will write to the LEDCTRL reg based on a passed mask.\n");
}
...
void print_menu()
{
	printf("\nPlease choose which test to run.\n"
	       "1. read led reg: read the value of led reg via read().\n"
	       "2. WR to ctrl leds: writes a command for led ctrl then reads.\n"
	       "3. exit\n"
	       "Enter number: ");
	fflush(stdout);
}
...
int read_test(int fd)
{
	...
	if (ret < (ssize_t)U32_SIZE) {
		noerr_msg("read_test: read did not read full bytes");
		return FAILURE;
	}

	printf("led control reg value from read: %X\n", inbuff);
	return SUCCESS;
}
...
int wr_test(int fd, led_opts_s *led_opts)
{
	...
	int led_num;
	int ret;
	uint8_t packed_led_opts = 0;

	led_num = get_led_to_set(stdin);
	if (led_num == FAILURE) {
		noerr_msg("get_led_to_set failed in wr_test.");
		return FAILURE;
	}

	ret = get_led_options(stdin, led_opts, led_num);
	if (ret == FAILURE) {
		noerr_msg("get_led_options failed in wr_test.");
		return FAILURE;
	}

	/* pack led options into a single byte for transfer in write */
	switch (led_num) {
	case LED0:
		packed_led_opts |= PACK_LED0(led_opts->led0_opt);
	break;

	case LED1:
		packed_led_opts |= PACK_LED1(led_opts->led1_opt);
	break;

	case LED2:
		packed_led_opts |= PACK_LED2(led_opts->led2_opt);
	break;

	case LED3:
		packed_led_opts |= PACK_LED3(led_opts->led3_opt);
	break;
	...
	default:
		errno = EINVAL;
		err_msg("wr_test: invalid led num");
		return FAILURE;
	}

	/* write packed byte to driver to set LEDCTRL to desired settings */
	ret = write(fd, &packed_led_opts, U8_SIZE);
	if (errno) {
		err_msg("write failed in wr_test.");
		return ret;
	}

	/* read value back and print it to check write is successful */
	ret = read(fd, &inbuff, U32_SIZE);
	if (errno) {
		err_msg("wr_test: read failed");
		return ret;
	}
